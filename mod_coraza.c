...
void coraza_log_cb(void *log, const void *data)
{
    const char *msg;
...
int process_intervention(coraza_transaction_t *t, request_rec *r)
{
    ...
    int z = coraza_intervention(t, &intervention);
...
/*
 * Called only once. Used to initialise the Coraza
 *
 */
int coraza_apache_init(apr_pool_t *mp)
{
    //msc_set_connector_info(msc_apache->modsec, MSC_APACHE_CONNECTOR);

    apr_pool_cleanup_register(mp, NULL, coraza_module_cleanup, apr_pool_cleanup_null);

    //msc_set_log_cb(msc_apache->modsec, coraza_log_cb);
...
{
    //coraza_cleanup(msc_apache->modsec);
    // TODO?
}
 ...
 */
static void store_tx_context(coraza_t *msr, request_rec *r)
{
    apr_table_setn(r->notes, NOTE_MSR, (void *)msr);
}

static coraza_t *create_tx_context(request_rec *r)
{
    coraza_t *msr = NULL;
    coraza_conf_t *z = NULL;
    char *unique_id = NULL;

    z = (coraza_conf_t *)ap_get_module_config(r->per_dir_config,
                                              &coraza_module);

    msr = (coraza_t *)apr_pcalloc(r->pool, sizeof(coraza_t));
    if (msr == NULL)
    {
        return NULL;
    }
    ...
    {
        msr->t = coraza_new_transaction_with_id(z->waf, unique_id, (void *)r);
    }
    else
    {
        msr->t = coraza_new_transaction(z->waf, (void *)r);
    }
 ...
 */
static coraza_t *retrieve_tx_context(request_rec *r)
{
    coraza_t *msr = NULL;
    request_rec *rx = NULL;

    /* Look in the current request first. */
    msr = (coraza_t *)apr_table_get(r->notes, NOTE_MSR);
    if (msr != NULL)
    {
        msr->r = r;
        return msr;
    ...
    if (r->main != NULL)
    {
        msr = (coraza_t *)apr_table_get(r->main->notes, NOTE_MSR);
        if (msr != NULL)
        {
            msr->r = r;
            return msr;
    ...
    while (rx != NULL)
    {
        msr = (coraza_t *)apr_table_get(rx->notes, NOTE_MSR);
        if (msr != NULL)
        {
            msr->r = r;
            return msr;
...
static int coraza_hook_pre_config(apr_pool_t *mp, apr_pool_t *mp_log,
                               apr_pool_t *mp_temp)
{
    void *data = NULL;
    const char *key = "coraza-pre-config-init-flag";
    int first_time = 0;
    ...
    int ret = coraza_apache_init(mp);

    if (ret == -1)
    {
        ap_log_error(APLOG_MARK, APLOG_STARTUP, 0, NULL,
                     "Coraza: Failed to initialise.");
...
static int coraza_hook_post_config(apr_pool_t *mp, apr_pool_t *mp_log,
                                apr_pool_t *mp_temp, server_rec *s)
{
    void *data = NULL;
    const char *key = "coraza-post-config-init-flag";
    int first_time = 0;
    ...
    ap_log_error(APLOG_MARK, APLOG_NOTICE | APLOG_NOERRNO, 0, s,
                 "ModSecurity: %s configured.", CORAZA_APACHE_CONNECTOR);
...
#error "Currently in v1 connection can only be processed late."
*/
    coraza_process_connection(msr->t, client_ip,
                           client_port,
...
static int hook_request_late(request_rec *r)
{
    ...
    coraza_process_request_body(msr->t);
    it = process_intervention(msr->t, r);
    if (it != N_INTERVENTION_STATUS)
    {
        return it;
...
static int hook_log_transaction(request_rec *r)
{
    ...
    coraza_update_status_code(msr->t, r->status);
    coraza_process_logging(msr->t);
    it = process_intervention(msr->t, r);
    if (it != N_INTERVENTION_STATUS)
    {
        return it;
...
static void hook_insert_filter(request_rec *r)
{
    ...
    ap_add_input_filter("CORAZA_IN", msr, r, r->connection);
#endif
    ...
    ap_add_output_filter("CORAZA_OUT", msr, r, r->connection);
}
...
static int process_request_headers(request_rec *r, coraza_t *msr)
{
    /* process uri */
    {
        ...
        coraza_process_uri(msr->t, r->unparsed_uri, r->method, r->protocol + offset);
        it = process_intervention(msr->t, r);
        if (it != N_INTERVENTION_STATUS)
        {
            return it;
    ...
    /* add request headers */
    {
        ...
        for (i = 0; i < arr->nelts; i++)
        {
            ...
            coraza_add_request_header(msr->t, key, val);
        }
        coraza_process_request_headers(msr->t);
...
static void coraza_register_hooks(apr_pool_t *pool)
{
    ...
    ap_hook_pre_config(coraza_hook_pre_config, NULL, NULL, APR_HOOK_FIRST);
    ap_hook_post_config(coraza_hook_post_config, postconfig_beforeme_list,
    ...
    /* request body */
    ap_register_input_filter("CORAZA_IN", input_filter,
                             NULL, AP_FTYPE_CONTENT_SET);

    /* response body */
    ap_register_output_filter("CORAZA_OUT", output_filter,
                              NULL, AP_FTYPE_CONTENT_SET - 3);
}

module AP_MODULE_DECLARE_DATA coraza_module =
    {
        STANDARD20_MODULE_STUFF,
        coraza_hook_create_config_directory, // Per-directory configuration.
        coraza_hook_merge_config_directory,  // Merge handler for per-directory.
        NULL,                                // Per-server conf handler.
        NULL,                                // Merge handler for per-server
                                             // configurations.
        module_directives,
        coraza_register_hooks};