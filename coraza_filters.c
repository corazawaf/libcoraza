                          ...
                          ap_input_mode_t mode, apr_read_type_e block, apr_off_t nbytes)
{
    ...
    while (!APR_BRIGADE_EMPTY(pbbTmp))
    {
        ...
        coraza_append_request_body(msr->t, data, len);
...
apr_status_t output_filter(ap_filter_t *f, apr_bucket_brigade *bb_in)
{
    ...
    {
        ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, f->r->server,
                     "Coraza: Internal Error: msr is null in output filter.");
        ap_remove_output_filter(f);
        return send_error_bucket(msr, f, HTTP_INTERNAL_SERVER_ERROR);
    }
    ...
    /* response headers */
    {
        ...
        for (i = 0; i < arr->nelts; i++)
        {
            ...
            coraza_add_response_header(msr->t, key, val);
        }
        ...
        coraza_process_response_headers(msr->t, r->status, "HTTP 1.1");
    ...
    /* response body */
    {
             ...
             pbktIn = APR_BUCKET_NEXT(pbktIn))
        {
            ...
            coraza_append_response_body(msr->t, data, len);
        }
        coraza_process_response_body(msr->t);
