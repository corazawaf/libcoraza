...
def test_lifecycle():
    ...
    ret = _c.coraza_add_request_header(
        tx, hname, len(hname), hvalue, len(hvalue)
    )
    ...
    ret = _c.coraza_add_response_header(
        tx, cname, len(cname), cvalue, len(cvalue)
    )
    ...
    print(f"  Intervention: action={it.action!r} status={it.status}")
...
def test_request_body_from_file():
    ...
    _c.coraza_process_request_body(tx)
    _c.coraza_process_response_headers(tx, 200, "HTTP/1.1")
    _c.coraza_process_response_body(tx)
